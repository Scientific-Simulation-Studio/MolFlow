...
class VertWoeBinning:
    ...
    def binning(
        ...
        if audit_log_path:
            assert isinstance(self.secure_device,
                              HEU), "only HEU support audit log"
        ...
        if isinstance(self.secure_device, HEU):
            assert len(
                bin_names) == 2, "only support two party binning in HEU mode"
        ...
        for device in workers:
            ...
            if isinstance(self.secure_device, HEU):
                ...
                idx_obj = worker.slave_build_sum_indices(
