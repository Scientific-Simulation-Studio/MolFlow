...
class VertWoeBinningPyuWorker:
    ...
    def _chi_merge(
        ...
        def get_chi(bin1: Tuple[float, float], bin2: Tuple[float, float]):
            ...
            return (
                math.pow(bin1_positive - bin1_expt_positive, 2) /
                bin1_expt_positive
                + math.pow(bin1_negative - bin1_expt_negative, 2) /
                bin1_expt_negative
                + math.pow(bin2_positive - bin2_expt_positive, 2) /
                bin2_expt_positive
                + math.pow(bin2_negative - bin2_expt_negative, 2) /
                bin2_expt_negative
            )
    ...
    def _apply_chimerge(
        ...
        for f_idx in range(len(split_points)):
            if isinstance(split_points[f_idx], list):
                ...
                merged_split_points.append(
                    np.delete(split_points[f_idx], merged_idx))
    ...
    def master_work(self, data: pd.DataFrame) -> Tuple[np.ndarray, Dict]:
        ...
        if self.binning_method == "chimerge":
            bins_stat, split_points = self._apply_chimerge(
                bins_stat, split_points)
    ...
    def slave_build_sum_select(self, data: pd.DataFrame) -> np.ndarray:
        ...
        '''
        bins_idx, self.split_points, else_bins_idx = self._build_feature_bins(
            data)
    ...
    def slave_sum_bin(
        ...
        assert len(
            else_positive) == 0, f"len(else_positive) {len(else_positive)}"
