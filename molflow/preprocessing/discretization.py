...
class KBinsDiscretizer:
    ...
    def _concat_discretizer(self, ests: List[SkKBinsDiscretizer]) -> SkKBinsDiscretizer:
        ...
        discretizer.bin_edges_ = np.concatenate(
            [est.bin_edges_ for est in ests])
        ...
        if self._strategy == 'uniform':
            ...
            if isinstance(df, HDataFrame):
                ...
                if df.partition_way == PartitionWay.HORIZONTAL:
                    ...
                    hdfs = [
                        HDataFrame(aggregator=aggregator,
    ...
    def transform(
        ...
        assert hasattr(
            self, '_discretizer'), 'Discretizer has not been fit yet.'
        self._check_dataframe(df)
        ...
        if isinstance(df, (HDataFrame, VDataFrame)):
            ...
            if df.partition_way == PartitionWay.HORIZONTAL:
                ...
                for part in df.partitions:
                    ...
                    est_part.n_bins_ = self._discretizer.n_bins_[
                        start_idx:end_idx]
