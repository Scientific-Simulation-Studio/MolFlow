...
class MinMaxScaler:
    ...
    def _transform(
        ...
        for device, part in df.partitions.items():
            ...
            scaler.fit(
                np.stack([self._scaler.data_min_[mask],
                         self._scaler.data_max_[mask]])
            )
...
class StandardScaler:
    ...
    def _fit_horizontal(
        ...
        if self._with_std:
            ...
            vars = [part.data.device(_cal_var)(mean, part.data)
    ...
    def _concatenate_scaler(self, scalers: List[SkStandardScaler]) -> SkStandardScaler:
        ...
        if self._with_std:
            scaler.scale_ = np.concatenate(
                [scaler.scale_ for scaler in scalers])
    ...
    def fit(
        ...
        if isinstance(df, MixDataFrame):
            if df.partition_way == PartitionWay.HORIZONTAL:
                ...
                parts_list = [list(part.partitions.values())
