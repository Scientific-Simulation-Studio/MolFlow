    ...
    def psi_join_csv(
        ...
        def send_proc():
            ...
            while read_bytes < in_file_bytes:
                current_read_bytes = min(
                    max_read_bytes, in_file_bytes - read_bytes)
                ...
                packed_bytes = struct.pack(
                    f'?i{len(current_read)}s', False, len(
        ...
        def recv_proc():
            ...
            while True:
                ...
                r1, r2, r3 = struct.unpack(
                    f'?i{len(recv_bytes)-8}s', recv_bytes)
                assert r2 == len(r3), f'invalid recv msg {r2}!={len(r3)}'
                # check if last batch
                if r1:
                    logging.warning(
                        f"rank:{self.rank} recv last {len(recv_bytes)}")
                    break
                out_file.write(r3)
    ...
    def _place_arguments(self, *args, **kwargs):
        def place(obj):
            if isinstance(obj, DeviceObject):
                ...
                meta, * \
                    refs = self.io.make_shares(obj, spu.Visibility.VIS_PUBLIC)
    ...
    def __call__(
        ...
        *,
        ...
        def wrapper(*args, **kwargs):
            ...
            fn, kwargs = _argnames_partial_except(
                func, static_argnames, kwargs)

            # convert every args to SPU objects.
            args, kwargs = self._place_arguments(*args, **kwargs)

            (meta_args, meta_kwargs) = jax.tree_util.tree_map(
                lambda x: x.meta if isinstance(
                    x, SPUObject) else x, (args, kwargs)
            )
