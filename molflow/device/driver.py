...
def reveal(func_or_object):
    ...
    for x in flatten_val:
        if isinstance(x, PYUObject):
            ...
            if x.is_plain:
                ref = x.device.get_participant(
                    x.location).decode.remote(x.data)
            else:
    ...
    for x in flatten_val:
        if isinstance(x, PYUObject) or isinstance(x, HEUObject):
            ...
            if isinstance(x.shares[0], ray.ObjectRef):
                shares = [all_object[cur_idx + i]
                          for i in range(x.device.world_size)]
    ...
    **kwargs,
    ...
    if parties is not None:
        ...
        if isinstance(parties, str):
            ...
            assert len(set(parties)) == len(
                parties), f'duplicated parties {parties}'
