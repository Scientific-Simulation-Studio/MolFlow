...
def _configure_deprecated_callbacks(
    ...
    logging.warn(
        f'Old style callback is deprecated.  See: {link}', UserWarning)
    # Most of legacy advanced options becomes callbacks
    if early_stopping_rounds is not None:
        callbacks.append(
            callback.early_stop(
                early_stopping_rounds, maximize=maximize, verbose=bool(
                    verbose_eval)
            )
        )
...
def _train_internal(
    ...
    if xgb_model is None:
        ...
        bst = FedBooster(params, [dtrain] + [d[0]
                         for d in evals], model_file=xgb_model)
    ...
    if is_new_callback:
        ...
        if verbose_eval:
            ...
            callbacks.append(
                xgb_callback.EvaluationMonitor(period=verbose_eval))
    ...
    for i in range(start_iteration, start_iteration + num_boost_round):
        ...
        bst.federate_update(params, dtrain, hdata,
                            global_bin, iter_round=i, fobj=obj)
    ...
    if bst.attr('best_score') is not None:
        ...
        bst.set_attr(best_ntree_limit=str(
            (bst.best_iteration + 1) * num_parallel))
