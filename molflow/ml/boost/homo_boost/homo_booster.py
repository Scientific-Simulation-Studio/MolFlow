...
class SFXgboost:
    def __init__(self, server, clients):
        ...
        for client in self.clients:
            self._workers.append(
                HomoBooster(device=client, clients=self.clients,
                            server=self.server)
            )
        self._workers.append(
            HomoBooster(device=self.server,
                        clients=self.clients, server=self.server)
        )
        for worker in self._workers:
            worker.initialize(
                {worker.device: worker.data for worker in self._workers})
    ...
    def save_model(self, model_path: Dict):
        ...
        for worker in self._workers:
            if worker.device in model_path.keys():
                res[worker.device] = worker.save_model(
                    model_path[worker.device])
        reveal(res)
    ...
    def dump_model(self, model_path: Dict):
        ...
        for worker in self._workers:
            if worker.device in model_path.keys():
                res[worker.device] = worker.dump_model(
                    model_path[worker.device])
        reveal(res)
