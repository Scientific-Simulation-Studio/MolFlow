...
class DecisionTree(object):
    ...
    def fit(self):
        ...
        logging.debug(
            'begin to fit local decision tree, tree idx {}'.format(
                self.tree_id)
        )
        ...
        for dep in range(tree_height):
            ...
            for batch_id, i in enumerate(
                ...
                cur_data_frame = self.cur_layer_datas[i: i +
    ...
    def update_tree(
        ...
        """
        logging.debug(
            'updating tree_node, cur layer has {} node'.format(
                len(cur_to_split))
        )
        ...
        for idx in range(len(cur_to_split)):
            ...
            left_data = cur_data_frame[cur_data_frame[best_split_col]
                                       < best_split_bin]
            left_node = Node(
                id=l_id,
                sum_grad=l_g,
    ...
    def save_xgboost_model(self, model_path: str, tree_nodes: List[Node]):
        ...
        for node in tree_nodes:
            ...
            tree_param["default_left"].append(
                True if node.missing_dir == -1 else False)
            ...
            tree_param["split_indices"].append(
                node.fid if node.fid is not None else 0)
            tree_param["split_type"].append(0)
            tree_param["sum_hessian"].append(node.sum_hess)

        json_model["learner"]["attributes"]["best_iteration"] = str(
            self.iter_round)
        ...
        json_model["learner"]["gradient_booster"]["model"]["trees"].append(
            tree_param)
