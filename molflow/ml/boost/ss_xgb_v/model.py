...
class XgbModel:
    ...
    def predict(
        ...
        pred = self.spu(
            lambda ps, base: (jnp.sum(jnp.concatenate(
                ps, axis=0), axis=0) + base).reshape(-1, 1)
        )(preds, self.base)
...
class Xgb:
    ...
    def _update_pred(self, tree: Dict[PYU, PYUObject], weight: SPUObject) -> None:
        ...
        current = self.spu(split_fn.predict_tree_weight)(
            weight_selects, weight)
    ...
    def _prepare(
        ...
        assert len(
            y_shape) == 1 or y_shape[1] == 1, "label only support one col"
        ...
        self.workers = [Worker(idx, device=pyu)
                        for idx, pyu in enumerate(x.partitions)]
        self.x = x.partitions
    ...
    def _tree_setup(self) -> None:
        ...
        self.spu_context = self.spu(
            split_fn.tree_setup, static_argnames=(
                "objective", "samples", "subsample")
        )(
            self.spu_context,
            self.pred,
            col_buckets_choices,
    ...
    def train(
        ...
        while len(model.trees) < self.trees:
            ...
            logging.info(
                f"epoch {len(model.trees) - 1} time {time.time() - start}s")
