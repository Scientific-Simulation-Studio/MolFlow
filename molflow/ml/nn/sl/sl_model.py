...
class SLModel:
    def __init__(
        self,
        base_model_dict: Dict[Device,
                              Callable[[], 'tensorflow.keras.Model']] = {},
        device_y: PYU = None,
    ...
    def handle_data(
        ...
        if dataset_builder is None:
            ...
            assert len(set(parties_length)
                       ) == 1, "length of all parties must be same"
    ...
    def fit(
        ...
        if dp_spent_step_freq is not None:
            assert isinstance(dp_spent_step_freq,
                              int) and dp_spent_step_freq >= 1
    ...
    def predict(
        ...
        for step in range(0, predict_steps):
            ...
            if verbose > 0:
                pbar.update(1)
            y_pred = self._workers[self.device_y].predict(
                *hiddens, compress=compress)
    ...
    def evaluate(
        ...
        for step in range(0, evaluate_steps):
            ...
            if verbose > 0:
                pbar.update(1)
            metrics = self._workers[self.device_y].evaluate(
                *hiddens, compress=compress)
    ...
    def save_model(
        ...
        for device, worker in self._workers.items():
            ...
            base_model_dir, base_model_name = base_model_path[device].rsplit(
                "/", 1)
