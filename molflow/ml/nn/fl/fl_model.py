...
class FLModel:
    def __init__(
        ...
        model: Union['TorchModel', Callable[[],
                                            'tensorflow.keras.Model']] = None,
        aggregator=None,
        strategy='fed_avg_w',
        consensus_num=1,
        backend="tensorflow",
    ...
    def initialize_weights(self):
        ...
        for device, worker in self._workers.items():
            weights = initial_weight.to(
                device) if initial_weight is not None else None
    ...
    def handle_data(
        ...
        if sampling_rate > 1.0:
            sampling_rate = 1.0
            logging.warn(
                "Batch size is too large it will be set to the data size")
        # check batch size
        for length in parties_length.values():
            batch_size = math.floor(length * sampling_rate)
            assert (
        ...
        for epoch in range(epochs):
            ...
            for step in range(0, self.steps_per_epoch, aggregate_freq):
                ...
                for device, worker in self._workers.items():
                    client_params = (
                        model_params.to(
                            device) if model_params is not None else None
                    )
            ...
            for local_metric in local_metrics:
                history.record_local_history(
                    party=device.party, metrics=local_metric)
            ...
            if epoch % validation_freq == 0 and valid_x is not None:
                ...
                history.record_global_history(
                    metrics=global_eval.values(), stage="val")
                # save checkpoint
    ...
    def save_model(
        ...
        if isinstance(model_path, str):
            model_path = {
                device: model_path for device in self._workers.keys()}
        ...
        for device, worker in self._workers.items():
            ...
            device_model_path, device_model_name = model_path[device].rsplit(
                "/", 1)
            if is_test:
            ...
            res.append(
                worker.save_model(os.path.join(
                    device_model_path, device_model_name))
            )
        wait(res)
    ...
    def load_model(
        ...
        for device, worker in self._workers.items():
            ...
            res.append(
                worker.load_model(os.path.join(
                    device_model_path, device_model_name))
            )
        wait(res)
