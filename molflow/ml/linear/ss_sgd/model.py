...
class SSRegression:
    ...
    def _pre_check(
        ...
        infeed_rows = int((infeed_rows + batch_size - 1) /
    ...
    def _epoch(self, spu_w: SPUObject, epoch_idx: int) -> SPUObject:
        ...
        for infeed_step in range(self.infeed_total_batch):
            if epoch_idx == 0:
                x, lr_total_batch = self._next_infeed_batch(
                    self.x, infeed_step)
                y, lr_total_batch = self._next_infeed_batch(
                    self.y, infeed_step)
                spu_x = self.spu(_concatenate, static_argnames=('axis'))(
                    [x.partitions[pyu].to(self.spu) for pyu in x.partitions], axis=1
                )
                spu_y = [y.partitions[pyu].to(self.spu)
                         for pyu in y.partitions][0]
    ...
    def fit(
        ...
        for epoch_idx in range(epochs):
            ...
            wait([spu_w])
            logging.info(
                f"epoch {epoch_idx + 1} times: {time.time() - start}s")
            # todo: do early stop
        self.batch_cache = {}
        self.spu_w = spu_w
    ...
    def predict(
        ...
        for infeed_step in range(infeed_total_batch):
            ...
            spu_x = [batch_x.partitions[pyu].to(
                self.spu) for pyu in batch_x.partitions]
            spu_pred = self.spu(
                _predict,
                static_argnames=('reg_type', 'sig_type',
        ...
        pred = self.spu(_concatenate, static_argnames=('axis'))(
            spu_preds, axis=0)
