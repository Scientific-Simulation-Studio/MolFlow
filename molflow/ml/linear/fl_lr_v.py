...
class FlLrVWorker(object):
    ...
    def init_train_data(
        ...
        rdm = RandomState(
            seed=shuffle_seed) if shuffle_seed is not None else None
    ...
    def generate_rand_mask(self, decode_frac: int) -> np.ndarray:
        ...
        mask_int = np.random.randint(
            np.iinfo(np.int64).min, np.iinfo(
                np.int64).max, size=(1, self.feat_size)
        )
        # save for decoding
    ...
    def get_weight(self) -> np.ndarray:
        return (
            np.concatenate((self.intercept, self.weight)
                           ) if self.has_y else self.weight
        )
...
class FlLogisticRegressionVertical:
    ...
    def __init__(
        ...
        self.workers = {device: PYUFlLrVWorker(
    ...
    def fit(
        ...
        assert len(
            y.partitions) == 1, f'One and only one participant should hold y.'
        ...
        for epoch in range(epochs):
            ...
            if loss <= tol:
                logging.info(
                    f'Stop training as loss is no greater than {tol} already.')
                return
