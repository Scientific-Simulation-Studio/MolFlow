...
class HESSLogisticRegression:
    ...
    def __init__(self, spu: SPU, heu_x: HEU, heu_y: HEU) -> None:
        ...
        self._heu_x = heu_x
        self._heu_y = heu_y
        self._scale = spu_fxp_precision(
            spu.cluster_def['runtime_config']['field'])
    ...
    def _data_check(
        ...
        for device, partition in x.partitions.items():
            if device.party == self._heu_x.sk_keeper_name():
                ...
                raise ValueError(
                    f"unexpected x's partition party {device.party}")
    ...
    def fit(
        ...
        assert isinstance(
            epochs, int) and epochs > 0, f"epochs should be integer > 0"
    ...
    def _predict(
        ...
        assert len(
            w.shape) == 1 or w.shape[1] == 1, "w should be list or 1D array"
