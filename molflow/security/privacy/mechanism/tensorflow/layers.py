...
class GaussianEmbeddingDP(EmbeddingDP):
    ...
    def __init__(
        ...
        self.delta = delta if delta is not None else min(
            1 / num_samples**2, 1e-5)
    ...
    def privacy_spent_rdp(self, step: int, orders: List = None):
        ...
        eps, _, opt_order = get_privacy_spent_rdp(
            orders, rdp, target_delta=self.delta)
    ...
    def privacy_spent_gdp(
        ...
        if sampling_type == 'poisson':
            ...
            raise ValueError(
                'the sampling_type must be "poisson" or "uniform".')
...
class LabelDP:
    ...
    def __call__(self, inputs: np.ndarray):
        ...
        if inputs.ndim == 1:
            ...
            p_ori = np.exp(self._eps) / \
                (np.exp(self._eps) + inputs.shape[-1] - 1)
            ...
            raise ValueError(
                'the dim of inputs in LabelDP must be less than 2.')
