    ...
    def spu_subtract_dispatcher(axis):
        def spu_subtract(obj_list1: List[np.ndarray], obj_list2: List[np.ndarray]):
            return jnp.concatenate(
                [obj_list1[i] - obj_list2[i]
                    for i in range(len(obj_list1))], axis
            )

        return spu_subtract

    return binary_op(
        spu_subtract_dispatcher(get_concat_axis(
            y1)), jnp.subtract, y1, y2, spu_device
    )
...
def unary_op(
    ...
    if y_len == 1:
        ...
        obj_list = [to(spu_device, partition)
                    for partition in y.partitions.values()]
        if simulate_double_value_replacer_handle:
...
def binary_op(
    ...
    if enable_local_compute_optimization:
        ...
        obj1_list = [to(spu_device, partition)
                     for partition in y1.partitions.values()]
        obj2_list = [to(spu_device, partition)
                     for partition in y2.partitions.values()]
...
def tss(y: FedNdarray, spu_device: Optional[SPU] = None):
    ...
    def spu_tss(obj1_list: List[np.ndarray], y_mean):
        sums = jnp.array([sum_of_difference_squares(obj, y_mean)
...
def residual_histogram(
    ...
    def hist_subtract_composition(y1, y2, bins, axis=None):
        if type(y1) == type(list):
            residual = jnp.concatenate([y1[i] - y2[i]
                                       for i in range(len(y1))], axis)
        else:
    ...
    if enable_local_compute_optimization:
        ...
        return spu_device(
            hist_concat_composition, static_argnames=(
                "axis_1", "axis_2", "bins")
