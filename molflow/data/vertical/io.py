...
def read_csv(
        ...
        if x:
            if isinstance(x, str):
                ...
                raise InvalidArgumentError(
                    f"Illegal type for keys,got {type(x)}")
        else:
            return []
    ...
    if spu is not None:
        if psi_protocl is None:
            psi_protocl = 'KKRT_PSI_2PC' if len(
                filepath) == 2 else 'ECDH_PSI_3PC'
    ...
    if drop_keys:
        for device, partition in partitions.items():
            ...
            if device_drop_key is not None:
                ...
                partitions[device] = partition.drop(
                    labels=device_drop_key, axis=1)
