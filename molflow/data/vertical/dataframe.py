...
class VDataFrame(DataFrameBase):
    ...
    def values(self):
        ...
        """
        return FedNdarray(
            partitions={pyu: part.values for pyu,
                        part in self.partitions.items()},
        ...
        for device, uri in fileuris.items():
            if device not in self.partitions:
                raise InvalidArgumentError(
                    f'PYU {device} is not in this dataframe.')
    ...
    def __setitem__(self, key, value):
        if isinstance(value, Partition):
            ...
            for pyu, col in key_index.items():
                self.partitions[pyu][col] = (
                    value.partitions[pyu] if isinstance(
                        value, VDataFrame) else value
                )
    ...
    def partition_columns(self):
        ...
        """
        assert len(
            self.partitions) > 0, 'Partitions in the dataframe is None or empty.'
        return {
