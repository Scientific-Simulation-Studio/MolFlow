...
class Partition(DataFrameBase):
    ...
    def __unwrap(self, args, kwargs):
        new_args = tree_map(lambda x: x.data if (
            type(x) == type(self)) else x, args)
        new_kwargs = tree_map(
    ...
    def astype(self, dtype, copy: bool = True, errors: str = "raise"):
        ...
        """
        return Partition(
            self.data.device(pd.DataFrame.astype)(
                self.data, dtype, copy, errors)
        )
    ...
    def iloc(self, index: Union[int, slice, List[int]]) -> 'Partition':
        ...
        """
        return Partition(
            self.data.device(lambda df, index: df.iloc[index])(
                self.data, index)
        )
    ...
    def __setitem__(self, key, value):
        ...
        self.data = self.data.device(_setitem)(
            self.data, key, value if not isinstance(
                value, Partition) else value.data
        )
