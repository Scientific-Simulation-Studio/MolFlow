...
class TestDevicePSI2PC(DeviceTestCase):
    ...
    def test_single_col(self):
        ...
        pd.testing.assert_frame_equal(
            sf.reveal(da).reset_index(drop=True), expected)
        ...
        pd.testing.assert_frame_equal(
            sf.reveal(db).reset_index(drop=True), expected)
    ...
    def test_different_cols(self):
        ...
        pd.testing.assert_frame_equal(
            sf.reveal(da).reset_index(drop=True), expected_a)
        pd.testing.assert_frame_equal(
            sf.reveal(db).reset_index(drop=True), expected_b)
...
class TestDevicePSI3PC(DeviceTestCase):
    ...
    def test_single_col(self):
        ...
        pd.testing.assert_frame_equal(
            sf.reveal(dc).reset_index(drop=True), expected)
...
class TestDevicePSIJoin(DeviceTestCase):
    ...
    def test_psi_join_df(self):
        ...
        da, db = self.spu.psi_join_df(
            select_keys, [self.da, self.db], 'bob', 'bob')
        ...
        pd.testing.assert_frame_equal(
            sf.reveal(da).reset_index(drop=True), result_a)
        pd.testing.assert_frame_equal(
            sf.reveal(db).reset_index(drop=True), result_b)
    ...
    def test_psi_join_csv(self):
        ...
        self.spu.psi_join_csv(select_keys, input_path,
                              output_path, 'alice', 'alice')
