...
from molflow.stats import psi_eval
from molflow.stats.core.utils import equal_range
...
class TestPSI(DeviceTestCase):
    ...
    def test_psi(self):
        score_1 = reveal(
            psi_eval(self.y_actual, self.y_expected_1, self.split_points))
        score_2 = reveal(
            psi_eval(self.y_actual, self.y_expected_2, self.split_points))
        true_score_2 = (0.5 - 0.8) * np.log(0.5 / 0.8) + \
            (0.5 - 0.2) * np.log(0.5 / 0.2)
